...
use \app\modelos\pacientes\BuscarAvancesEntrevistaPsicologicaporPacienteNING_ID;
use \app\nucleo\{Config};
use \app\modelos\vista\Vista;

class ControladorENTREVISTAPSICOLOGICA
{
	...
	(
		...
		Entrada $Entrada,
		Redirigir $Redirigir,
		BuscarAvancesEntrevistaPsicologicaporPacienteNING_ID $BuscarAvancesEntrevistaPsicologicaporPacienteNING_ID,
		Config $Config,
		Vista $Vista
	)
	{
		...
		$this->_avance = $BuscarAvancesEntrevistaPsicologicaporPacienteNING_ID;
		$this->_config = $Config;
		$this->_vista = $Vista;
	}
		...
		if($this->_revisartoken->revisar($_POST['TOKEN']))
		{
			if($_POST['NING_ID'] !== '0')
			{
				...
				foreach ($this->_avance->obtener($_POST['NING_ID']) as $key => $value)
				{
					if($value == 0)
					{
						$this->_vista->ver('psicologia/CapturarENTREVISTA'.$key);
						break;
					}
